...
const app = express();
const port = 3000;
const JWT_SECRET = 'seu-segredo-jwt-super-secreto';
...
/**
 * Helper para atualizar status de agendamentos (expiração/conclusão automática)
 */
async function atualizarStatusAgendamentos(connection) {
  try {
    // 1. Marca como 'concluido' os agendamentos 'confirmados' cuja data de fim já passou.
    ...
    // 2. Marca como 'cancelado' os agendamentos 'pendentes' cuja data de início já passou (expirados).
    await connection.query(`
      UPDATE agendamentos 
      SET status_agendamento = 'cancelado',
          observacoes = CONCAT(IFNULL(observacoes, ''), ' [Cancelado automaticamente por expiração]')
      WHERE status_agendamento = 'pendente' 
      AND data_hora_inicio < NOW()
    `);
    
  } catch (error) {
    console.error("Erro ao atualizar status automáticos:", error);
    // Não lança erro para não bloquear a listagem se falhar o update
  }
}
...
app.get('/api/professor/agendamentos', autenticarToken, async (req, res) => {
  ...
  try {
    ...
    await atualizarStatusAgendamentos(connection);
...
app.get('/api/admin/agendamentos', autenticarToken, async (req, res) => {
    ...
    try {
        ...
        // Atualiza status também para o admin
        await atualizarStatusAgendamentos(connection);
...
app.get('/api/tecnico/agendamentos/pendentes', autenticarToken, async (req, res) => {
  ...
  try {
    ...
    // Atualiza status para garantir que o técnico não veja pendentes expirados
    await atualizarStatusAgendamentos(connection);
