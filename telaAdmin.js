document.addEventListener('DOMContentLoaded', function () {
    ...
    async function iniciarCarregamentoDados() {
        try {
            ...
            document.getElementById('lista-agendamentos-tbody').innerText = `<tr><td colspan="6" data-label="Erro" class="text-center text-danger">Erro ao carregar dados.</td></tr>`;
            document.getElementById('lista-usuarios-tbody').innerText = `<tr><td colspan="4" data-label="Erro" class="text-center text-danger">Erro ao carregar dados.</td></tr>`;
            document.getElementById('lista-estoque-tbody').innerText = `<tr><td colspan="5" data-label="Erro" class="text-center text-danger">Erro ao carregar dados.</td></tr>`;
        }
    }
    ...
    function renderAgendamentos() {
        ...
        if (appState.agendamentos.length === 0) {
            tbody.innerText = `<tr><td colspan="6" data-label="Aviso" class="text-center">Nenhum agendamento encontrado.</td></tr>`;
            return;
        }

        tbody.innerText = appState.agendamentos.map(aula => {
    ...
    function renderUsuarios() {
        ...
        if (appState.usuarios.length === 0) {
            tbody.innerText = `<tr><td colspan="4" data-label="Aviso" class="text-center">Nenhum usuário cadastrado.</td></tr>`;
            return;
        }

        tbody.innerText = appState.usuarios.map(user => `
            <tr data-id="${user.id}">
    ...
    function renderEstoque() {
        ...
        if (appState.materiais.length === 0) {
            tbody.innerText = `<tr><td colspan="5" data-label="Aviso" class="text-center">Nenhum item no estoque.</td></tr>`;
            return;
        }

        tbody.innerText = appState.materiais.map(item => `
            <tr data-id="${item.id}">
    ...
    function renderEstatisticas() {
        ...
        const totalConcluidos = usuarios.filter(a => a.status_agendamento === "concluido").length;
        document.getElementById("stat-concluidos").innerText = totalConcluidos;

        // Agendamentos cancelados
        const totalCancelados = usuarios.filter(a => a.status_agendamento === "cancelado").length;
